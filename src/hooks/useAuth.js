...
export default function useAuth( setLoading, setMessage, setStatus ) {
    ...
    const isAuthenticated = async () => {
        ...
        if(!token || token === null || token === undefined || token === '') { return {status : false, payload : null}; }
        
        try {
            const status = await axios.get('/auth/payload', {params : { token }});
            
            if(status.data.payload.user.accessLvL !== 'root' && status.data.payload.user.accessLvL !== 'mid') {
                eraseCookie('auth');
                eraseCookie('token');
